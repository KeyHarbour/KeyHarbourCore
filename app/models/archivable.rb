module Archivable
  ...
  included do
    ...
    scope :active,          -> { where(status: Organization.statuses[:active]) }
    scope :not_archived,    -> { where.not(status: Organization.statuses[:archived]) }
    scope :disabled,        -> { where(status: Organization.statuses[:disabled]) }
    scope :archived,        -> { where(status: Organization.statuses[:archived]) }
